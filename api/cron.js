export default async function handler(req, res) {
  try {
    ...
    const GROQ_KEY = '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';
    const BOT_TOKEN = '8684401835:AAGdROhnSdcNUkCvvgNTXlYidNPS_yWncZE';
    ...
    const LANGUAGE_NAMES = { en:'English', zh:'Mandarin Chinese', vi:'Vietnamese', es:'Spanish', ru:'Russian', ar:'Arabic' };

    function sleep(ms) { return new Promise(r => setTimeout(r, ms)); }

    async function apiPost(hostname, path, headers, body) {
    ...
    async function groqCall(prompt) {
      const r = await apiPost('api.groq.com', '/openai/v1/chat/completions',
    ...
    async function saveGameOfDay(game) {
      ...
      await apiPost('ehjhsbrcbtqcvmgzjzkm.supabase.co', '/rest/v1/game_of_day',
    ...
    async function savePicks(free_pick, premium_picks, game_data) {
      await apiPost('ehjhsbrcbtqcvmgzjzkm.supabase.co', '/rest/v1/daily_picks',
        { 'apikey': SUPABASE_KEY, 'Authorization': 'Bearer '+SUPABASE_KEY, 'Prefer': 'resolution=merge-duplicates' },
    ...
    async function postTG(chatId, message) {
      return apiPost('api.telegram.org', '/bot'+BOT_TOKEN+'/sendMessage', {},
    ...
    // Always generate and save picks
    const { games, sport } = await getTodaysGames();
    console.log('Games:', games.length, sport);
    ...
    return res.status(200).json({ ok: true, date: today, games: games.length });
  } catch(e) {
