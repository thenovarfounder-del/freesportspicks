export default async function handler(req, res) {
  try {
    ...
    const GROQ_KEY = process.env.GROQ_KEY || "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";
    ...
    async function generatePicks(games, sport) {
      ...
      const freeMatch = text.match(/FREE PICK:([sS]*?)(?=PREMIUM CARD|PICK #1:|$)/i);
      const premiumMatch = text.match(/PREMIUM CARD[sS]*|PICK #1:[sS]*/i);
      return {
    ...
    async function saveGameOfDay(game) {
      ...
      await apiPost(
        ...
        { date: today, home_team: game.strHomeTeam, away_team: game.strAwayTeam, sport: game.strSport, league: game.strLeague, game_time: easternTime }
      );
    ...
    async function generateCryptoSignals() {
      ...
      const premiumSignals = parts[1] ? 'LOCKED\nSignal 2 - LOCKED\nSignal 3 - Unlock: t.me/FreeSportsPicksProBot' : 'LOCKED\nSignal 2 - LOCKED\nSignal 3 - Unlock: t.me/FreeSportsPicksProBot';
